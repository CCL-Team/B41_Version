/**************************************************************************
 Program Title:   mp_top_get_get_doc
 
 Object name:     mp_top_get_get_doc
 Source file:     mp_top_get_get_doc.prg
...
002 04/17/2018 Michael Mayes        Proactive Change to date field gathered to show last doc time.
002 05/27/2020 Michael Mayes        221347    Changing form name hard coding
*************END OF ALL MODCONTROL BLOCKS* *************************************************************************************/
drop program mp_top_get_get_doc:dba go
create program mp_top_get_get_doc:dba
...
**************************************************************/
;Build and Cert and prod?
declare form_ref_id      = f8 with protect, constant(7601360159.00) ;Magic number (we can't query for the name here, it changes)

;Prod
;declare form_ref_id      = f8 with protect, constant(4657905355.00) ;Magic number (we can't query for the name here, it changes)

declare prim_evnt_cd     = f8 with protect, constant(uar_get_code_by('DISPLAYKEY', 18189, 'PRIMARYEVENTID'))
