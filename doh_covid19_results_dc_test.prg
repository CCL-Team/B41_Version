/***********************************************************************************************************
 Program Title:     DOH_COVID19_RESULTS_DC_test.prg
 Create Date:       03/11/2020
 Object name:       DOH_COVID19_RESULTS_DC_TEST
 Source file:       DOH_COVID19_RESULTS_DC_TEST.prg
 MCGA:
 OPAS:
 Purpose:https:
...
*************************************************************************************************************
 Clone from jwd_PALLIATIVE_CARE_METRICS.prg
  ...
  This report is only for ***DC DEPT OF HEALTH*** looking at OP,OP Message,Recurring Clinic,
  and Clinic encounters for the purpose of Covid 19 lab reporting as well as Case reporting
  and looks for DTAs:
 
 
 by form date and location on the prompt.
...
WRAPPERS - doh_covid19_results_wrapper - file
doh_covid19_case_wrapper - email
...
001    03/25/2024       Michael Mayes           PEND                Preventing filesize seqfault.
 
*************END OF ALL MODCONTROL BLOCKS* *******************************************************************/
drop program doh_covid19_results_dc_test go
create program doh_covid19_results_dc_test
...
SET FILE_NAME =  concat("/cerner/d_p41/cust_output_2/doh_covid_results/medstar_dc-elrambtest-",format(cnvtdatetime(curdate,curtime3),"YYYYMMDDhhmmss;;Q"), ".csv")
    ;build2("medstar_DCTEST", dataDate,".csv")
...
IF($TYPE = 3);EMAILING OF REPORT
    ...
    SET EMAIL_SUBJECT = build2("DC DOH ELR-TEST Report")
    ...
    SET EMAIL_BODY = concat("jwd_covid_dctst_", format(cnvtdatetime(curdate, curtime3),"YYYYMMDDhhmmss;;Q"), ".dat")
 
    DECLARE FILENAME = VC
            WITH  NOCONSTANT(CONCAT("jwd_covid_dctst_",
    ...
    IF ($TYPE = 3 and CURDOMAIN = PRODUCTION_DOMAIN)
        Select into (value(EMAIL_BODY))
            build2("The DC DOH ELR TEST Report is attached to this email.", char(13), char(10), char(13), char(10),
                "Date Range: ", $START_DT , " to ", $END_DT , char(13), char(10), char(13), char(10),
                "Run date and time: ",format(cnvtdatetime(curdate, curtime3),"MM/DD/YYYY hh:mm;;Q"), char(13), char(10), char(13),char(10))
...
;               2238539103 ;Respiratory Virus Pnl PCR
...
;join cep where cep.event_id = OUTERJOIN(c3.event_id)
...
;   RS->QUAL[patients]->oc_stat = "UNK"
        ...
        if(rs->QUAL[patients]->OrderId = 0.0)
        rs->QUAL[patients]->Order_PROVIDER = "Unassigned, Unassigned"
        rs->QUAL[patients]->npi = "ZZZZZZ"
        endif
        endif
;       if(c3.event_cd = 2282064783)
;       rs->QUAL[patients]->o_loinc = "94563-4"
;       endif
...
and c.event_cd in (
    ...
    and c.event_end_dt_tm  > CNVTDATETIME(CNVTDATE( 04072018), 0700)
...
;
; PLAN D1
; JOIN P WHERE P.PERSON_id = rs->qual[d1.seq].PersonId
; AND P.active_ind = 1
;                 AND P.PROBLEM_ID != 0.00
;                AND P.CONFIRMATION_STATUS_CD = 3305.00 ;CONFIRMED
;                AND P.LIFE_CYCLE_STATUS_CD = 3301.00 ;ACTIVE
;                AND P.END_EFFECTIVE_DT_TM > cnvtdatetime(curdate,curtime3)
;  join n where n.nomenclature_id = P.nomenclature_id
;       and n.source_string_keycap in (
;
;                                   "*DIABET*",
;                                   "*OBESIT*",
;                                   "*HYPERTENSION*"
...
;
;
;   RS->QUAL[D1.SEQ].UNDER_COND= "Y"
;   IF(n.source_string_keycap = "*DIABET*")
;   RS->QUAL[D1.SEQ].DIABETES = N.source_string
;   ELSEIF(n.source_string_keycap = "*OBESIT*")
;   RS->QUAL[D1.SEQ].OBESITY = N.source_string
;   ELSEIF(n.source_string_keycap = "*HYPERTENSION*")
;   RS->QUAL[D1.SEQ].HYPERTENSION = N.source_string
;   ENDIF
;
...
;and c.event_cd in (704719.00,  102264550.00,  823528831.00,  823678831.00)
;;and c.event_end_dt_tm > rs->qual[d.seq].reg_dt
;and c.event_end_dt_tm > cnvtlookbehind("1,W")
;
;detail
;
;rs->QUAL[d.seq].homeless = c.result_val
;
;
;with nocounter, time = 200
;/********************************************************************************************
...
; plan d
;
;join o where o.encntr_id = rs->qual[d.seq].EncntrId
;
;join oc where o.catalog_cd = oc.catalog_cd and oc.catalog_cd in (
;
;3883419    ;Ventilator Settings, Left Lung (DLV)
;,3883421   ;Ventilator Settings, Right Lung (DLV)
;,101881089 ;Ventilator Settings Pressure Control Ventilation  Constant I Time
;,102247134 ;Ventilator Settings Mandatory Minute Ventilation
;,101847619 ;Ventilator Settings Airway Pressure Release Ventilation
;,102247122 ;Ventilator Settings Continuous Positive Airway Pressure
;;,101814593    ;NM Lung Scan Ventilation ONLY
;,102247176 ;Ventilator Settings Pressure Support Ventilation
;,101847640 ;Ventilator Settings Assist/Control and Continuous Mandatory Ventilation
;,101847733 ;Ventilator Settings BiLevel
;,102247174 ;Ventilator Settings Pressure Regulated Volume Control
;,102246836 ;Ventilator Settings Synchronized Intermittent Mandatory Ventilation
;,101976358 ;Ventilator Weaning per Protocol
;,102277461 ;Ventilator Settings Pressure Control Constant  IE Ratio
;,101847659 ;Ventilator Settings High Frequency Oscillatory Ventilation
;,258411336 ;DME Ventilator for Discharge
;,102284053 ;Ventilator Settings VS
;,199781347 ;Pediatric Ventilator Settings SIMV/VC/AF/PS
;,199782108 ;Pediatric Ventilator Settings SIMV/PC/PS
;,102283728 ;Neonatal Ventilator Settings NIPPV
;,102283729 ;Neonatal Ventilator Settings HFJV 
;,102283730 ;Neonatal Ventilator Setting CPAP
;,102283737 ;Neonatal Ventilator Settings PSVG
;,102283738 ;Neonatal Ventilator Setting Nasal CPAP
;,102283745 ;Pediatric Ventilator Settings PS+PEEP
;,102283750 ;Neonatal Ventilator Setting SIPAP(Biphasic)
;,102283754 ;Neonatal Ventilator Settings SIMV
;,102283756 ;Pediatric Ventilator Settings Non-Invasive Positive Pressure Ventilation
;,102283759 ;Neonatal Ventilator Settings HFOV
;,102283766 ;Neonatal Ventilator Settings AC
;,102283767 ;Pediatric Ventilator Settings HFOV
;,102283775 ;Pediatric Ventilator Settings APRV
;,102283779 ;Neonatal Ventilator Settings CMV
;,102283998 ;Ventilator Settings PAV
;,199782546 ;Pediatric Ventilator Settings SIMV/VC/PS
;,1777122573    ;Ventilator Settings PRVC SIMV
;,1777573239    ;Ventilator Management Protocol
;,1869907305    ;Cpap Ventilation Cpap Initiation & Mgmt
;,2156015329    ;Ventilator Management (Adult Protocol)
;,2156015345    ;Ventilator Management (Neonate/Pediatric)
;,2153195583    ;Pediatric Ventilator Settings PRVC
;,1879552173    ;Adaptive Servo Ventilation ASV OP only
;
;)
;detail
;
;rs->QUAL[d.seq].vent_order = "Y"
;rs->QUAL[d.seq].VENT_ORDER_NAME = OC.description
;
;
;with nocounter, time = 200
;/********************************************************************************************
...
Select into "nl:"
  ...
  PLAN d
   where SIZE(RS->QUAL,5) > 0         ; 001 Preventing a hit here where we have a zero
     and rs->qual[d.seq].PERSONID > 0 ; 001 Preventing a hit here where we have a zero
...
Select into "nl:"
  ...
  PLAN d
   where SIZE(RS->QUAL,5) > 0       ; 001 Preventing a hit here where we have a zero
     and rs->qual[d.seq].org_id > 0 ; 001 Preventing a hit here where we have a zero
 
  join A 
   WHERE A.parent_entity_id = rs->qual[d.seq].org_id
     and a.active_ind = 1
     and a.end_effective_dt_tm > cnvtdatetime(sysdate)
     and a.address_type_cd = 754.00
 
  JOIN PH 
   WHERE PH.parent_entity_id =outerjoin( A.parent_entity_id)
     AND PH.active_ind = outerjoin(1)
     and ph.beg_effective_dt_tm < outerjoin(cnvtdatetime(curdate,curtime))
     and ph.end_effective_dt_tm > outerjoin(cnvtdatetime(curdate,curtime))
...
;   IF(n.source_string_keycap = "*DIABET*")
;   RS->QUAL[D1.SEQ].DIABETES = "Y"
;   ELSEIF(n.source_string_keycap = "*ASTHMA*")
;   RS->QUAL[D1.SEQ].ASTHMA = "Y"
    IF(n.source_string_keycap = "*CHRONIC KIDNEY DISEASE*")
    RS->QUAL[D1.SEQ].CHRONIC_KID = n.source_string_keycap
    ELSEIF(n.source_string_keycap = "*IMMUNOSUPPRESSIVE DISEASE*")
    RS->QUAL[D1.SEQ].IMMUNO_DISEASE = n.source_string_keycap
;   ELSEIF(n.source_string_keycap = "CARDIO*")
;   RS->QUAL[D1.SEQ].CARDIO_DISEASE = "Y"
;   ELSEIF(n.source_string_keycap = "CO*")
;   RS->QUAL[D1.SEQ].COPD = "Y"
;   ELSEIF(n.source_string_keycap = "HYPERTENSION")
;   RS->QUAL[D1.SEQ].HYPERTENSION = "Y"
    ENDIF
...
detail
    ...
    IF(n.source_string_keycap = "*DIABET*")
    RS->QUAL[D1.SEQ].DIABETES = n.source_string_keycap
;   ELSEIF(n.source_string_keycap = "*ASTHMA*")
;   RS->QUAL[D1.SEQ].ASTHMA = "Y"
;   ELSEIF(n.source_string_keycap = "*CHRONIC KIDNEY DISEASE*")
;   RS->QUAL[D1.SEQ].CHRONIC_KID = "Y"
;   ELSEIF(n.source_string_keycap = "*IMMUNOSUPPRESSIVE DISEASE*")
;   RS->QUAL[D1.SEQ].IMMUNO_DISEASE = "Y"
    ELSEIF(n.source_string_keycap = "CARDIO*")
    RS->QUAL[D1.SEQ].CARDIO_DISEASE = n.source_string_keycap
;   ELSEIF(n.source_string_keycap = "CO*")
;   RS->QUAL[D1.SEQ].COPD = "Y"
    ELSEIF(n.source_string_keycap = "HYPERTENSION")
    RS->QUAL[D1.SEQ].HYPERTENSION = n.source_string_keycap
    ENDIF
...
;
;JOIN ce where ce.person_id = rs->qual[d.seq].PersonId
;and ce.event_cd = 34466527.00  ;Body Mass Index Dosing
;and ce.valid_until_dt_tm > cnvtdatetime(curdate,curtime)
;; and ce.event_end_dt_tm > cnvtlookbehind("90,D")
;
; and ce.event_id = (select max(ce2.event_id) from clinical_event ce2 where ce2.person_id = ce.person_id
; and ce2.event_cd = 34466527.00    ;Body Mass Index Dosing
; and ce2.valid_until_dt_tm > cnvtdatetime(curdate,curtime)
; ;and ce2.event_end_dt_tm > cnvtlookbehind("90,D")
; )
;order by ce.event_end_dt_tm desc
;detail
; if(cnvtreal(ce.result_val) > 34.9)
;rs->QUAL[D.SEQ].bmi = ce.result_val
;Endif
;
;with nocounter, format, time = 200
;
/********************************************************************************************
...
; if (size(rs->QUAL,5) > 0); AT LEAST ONE PATIENT FOUND ABOVE
 ...
 SELECT DISTINCT INTO VALUE(FILE_NAME)
  RESULT = if(rs->QUAL[D1.SEQ].Result != " ")
    ...
    , PT_PHONE = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].PT_PHONE))
    , ACCESSION = cnvtstring(cnvtreal(rs->QUAL[D1.SEQ].ACCESSION))
    ...
    , LAB_WHERE_TESTED = trim(SUBSTRING(1, 200, rs->QUAL[D1.SEQ].Perform_Location))
    ...
    , ORDERING_FAC_ADDR2 = trim(SUBSTRING(1, 50, rs->QUAL[D1.SEQ].fac_StreetAddr2))
    , ORDERING_FAC_CITY= trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].fac_City))
    , ORDERING_FAC_STATE = trim(SUBSTRING(1, 10, rs->QUAL[D1.SEQ].fac_State))
    ...
    , ORDERID = cnvtstringchk(rs->QUAL[D1.SEQ].EVENT_id)
    , LOINC = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].LOINC))
 
    , ORDER_DATE = format(RS->QUAL[d1.seq].ORDER_DTTM, "MM/DD/YYYY;;D")
    , TEST_CODE = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].o_loinc))
    , RESULT_CODE = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].loinc))
    ...
    , AOE_FACILITY = trim(SUBSTRING(1, 100, rs->QUAL[D1.SEQ].Clinic_Name))
    ;, AOE_FACILITY_CLIA = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].OrderingCLIA))
    , AOE_FACILITY_ZIP = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].fac_ZIP))
...
;   , DESCRIBE_SYMPTOMS = "NA"; trim(SUBSTRING(1, 100, rs->QUAL[D1.SEQ].visit_reason))
;   ;, VENT             = trim(substring(1,20,RS->QUAL[d1.seq].VENT))
;   ;, VENT_ORDER       = trim(substring(1,20,RS->QUAL[d1.seq].VENT_ORDER))
;   ;, VENT_ORDER_NAME      = trim(substring(1,50,RS->QUAL[d1.seq].VENT_ORDER_NAME))
;   , HCW               = "NA";trim(substring(1,20,RS->QUAL[D1.SEQ].oc_stat))
;   , FEVER         = "NA";trim(substring(1,10,RS->QUAL[d1.seq].FEVER_CD2))
;   , COUGH         = "NA";trim(substring(1,10,RS->QUAL[d1.seq].COUGH_CD2))
;   , SORE_THROAT   = "NA";trim(substring(1,10,RS->QUAL[d1.seq].SORE_THROAT_CD2))
;   , SORE_THROAT_FLU= "NA";trim(substring(1,10,RS->QUAL[d1.seq].ST_FLU_CD2))
;   , RHINORRHEA        = "NA";trim(substring(1,10,RS->QUAL[d1.seq].RHINO_cd))
;   , SHORT_OB      = "NA";trim(substring(1,10,RS->QUAL[d1.seq].SOB_cd))
;   , ACUTE_RESP_DIST= "NA";trim(substring(1,15,RS->QUAL[d1.seq].ARD_CD))
;   , HOMELESS       = "NA";trim(substring(1,15,RS->QUAL[d1.seq].homeless))
;   ;, INCARCERATED   = trim(substring(1,15,RS->QUAL[d1.seq].jail))
;   , LONGTERM_CARE  = "NA";trim(substring(1,15,RS->QUAL[d1.seq].LTC))
;   , UNDERLYING_COND = "NA";trim(substring(1,15,RS->QUAL[d1.seq].UNDER_COND))
;   , DIABETES = trim(substring(1,30,RS->QUAL[d1.seq].DIABETES))
;   , HYPERTENSION = trim(substring(1,30,RS->QUAL[d1.seq].HYPERTENSION))
;   , OBESITY= "NA";trim(substring(1,30,RS->QUAL[d1.seq].OBESITY))
;   , ADDITIONAL_NOTES= "NA";"TBD"
    ...
    where SIZE(RS->QUAL,5) > 0
      ...
      and rs->QUAL[d1.seq].ENCNTR_CD NOT IN (
    ...
    ;and (rs->QUAL[d1.seq].State = "DC" or rs->QUAL[d1.seq].fac_State = "DC" or rs->QUAL[d1.seq].State = "MD" or rs->QUAL[d1.seq].fac_State = "MD")
    ;and rs->QUAL[d1.seq].State != "MD" ;and rs->QUAL[d1.seq].fac_State = "DC");or rs->QUAL[d1.seq].State != "VA")
    and rs->QUAL[d1.seq].fac_State = "DC"
 ...
 if (size(rs->QUAL,5) > 0)
  ...
  RESULT = if(rs->QUAL[D1.SEQ].Result != " ")
    ...
    , LASTNAME = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].LastName))
    , FIRSTNAME = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].FirstName))
    , MRN = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].mrn))
    , CMRN = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].CMRN))
    , ENCOUNTER_TYPE = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].Encntr_Type))
    , GENDER = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].Gender))
    , DOB = SUBSTRING(1, 20, rs->QUAL[D1.SEQ].DOB)
    , RACE = SUBSTRING(1, 30, rs->QUAL[D1.SEQ].Race)
    , ETHNICITY = SUBSTRING(1, 30, rs->QUAL[D1.SEQ].ETHNIC_CD)
    ...
    , SERVICE_RESOURCE = trim(SUBSTRING(1, 50, rs->QUAL[D1.SEQ].SERVICE_RESOURCE))
    , ORDERING_FACILITY = trim(SUBSTRING(1, 100, rs->QUAL[D1.SEQ].Clinic_Name))
    , ORDERING_FAC_ADDR = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].fac_StreetAddr))
    , ORDERING_FAC_ADDR2 = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].fac_StreetAddr2))
    , ORDERING_FAC_CITY= trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].fac_City))
...
;   , DIABETES = trim(substring(1,30,RS->QUAL[d1.seq].DIABETES))
;   , HYPERTENSION = trim(substring(1,30,RS->QUAL[d1.seq].HYPERTENSION))
;   , CARDIOVASCULAR_DISEASE = trim(substring(1,30,RS->QUAL[d1.seq].CARDIO_DISEASE))
;   , IMMUNOSUPPRESSIVE_DISEASE = trim(substring(1,30,RS->QUAL[d1.seq].IMMUNO_DISEASE))
;   , CHRONIC_KIDNEY_DISEASE  = trim(substring(1,30,RS->QUAL[d1.seq].CHRONIC_KID))
;   , BMI  = trim(substring(1,30,RS->QUAL[d1.seq].bmi))
;   , OBESITY= "NA";trim(substring(1,30,RS->QUAL[d1.seq].OBESITY))
;   , ADDITIONAL_NOTES= "NA";"TBD"
...
;   , LONGTERM_CARE  = "NA";trim(substring(1,15,RS->QUAL[d1.seq].LTC))
;   , UNDERLYING_COND = "NA";trim(substring(1,15,RS->QUAL[d1.seq].UNDER_COND))
;   , DIABETES = "NA";trim(substring(1,30,RS->QUAL[d1.seq].DIABETES))
;   , HYPERTENSION = "NA";trim(substring(1,30,RS->QUAL[d1.seq].HYPERTENSION))
;   , OBESITY= "NA";trim(substring(1,30,RS->QUAL[d1.seq].OBESITY))
;   , ADDITIONAL_NOTES= "NA";"TBD"
    ...
    ;and (rs->QUAL[d1.seq].State = "DC" or rs->QUAL[d1.seq].fac_State = "DC")
    ;and rs->QUAL[d1.seq].State != "MD" ;and rs->QUAL[d1.seq].fac_State = "DC");or rs->QUAL[d1.seq].State != "VA")
    and rs->QUAL[d1.seq].Result != "TNP"
    and rs->QUAL[d1.seq].Result != "QNS"
    AND rs->QUAL[d1.seq].fac_State = "DC"
...
;;          CALL DCL(AIX_COMMAND,AIX_CMDLEN, AIX_CMDSTATUS)
        ...
        set EMAIL_SUBJECT = "COVID-19 DC DOH ELR TEST -No accounts found"
