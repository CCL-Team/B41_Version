/*************************************************************************
 Program Title:   SBSM ED Visits
 
 Object name:     14_st_sbsm_ed_visits
 Source file:     14_st_sbsm_ed_visits.prg
 
 Purpose:         Show some information about ED visits during
                  current pregnancy.
...
*************END OF ALL MODCONTROL BLOCKS* *******************************/
  drop program 14_st_sbsm_ed_visits:dba go
create program 14_st_sbsm_ed_visits:dba
...
select into 'nl:'
 ...
 where e.person_id           =  p_id
   and e.encntr_type_cd      =  309310  ;Emergency
...
detail
    data->ed_cnt = data->ed_cnt + 1
    
    stat = alterlist(data->qual, data->ed_cnt)
    
    
    data->qual[data->ed_cnt]->enc_id       = e.encntr_id
    data->qual[data->ed_cnt]->ed_dt_tm     = e.reg_dt_tm
    data->qual[data->ed_cnt]->ed_dt_tm_txt = trim(format(e.reg_dt_tm, 'MM-DD-YY'), 3)
...
if(data->ed_cnt > 0)
    set tmp_str = notrim(build2(wb, 'Number of ED Visits - ', trim(cnvtstring(data->ed_cnt), 3), wr, reol))
 
    for(looper = 1 to data->ed_cnt)
        set tmp_str = notrim(build2(tmp_str, 'ED Visit ', data->qual[looper]->ed_dt_tm_txt, reol))
    endfor
endif
