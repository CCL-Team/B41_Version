/***********************************************************************************************************
 Program Title:     DOH_COVID19_RESULTS_DC.prg
 Create Date:       03/11/2020
 Object name:       DOH_COVID19_RESULTS_DC
 Source file:       DOH_COVID19_RESULTS_DC.prg
 MCGA:
 OPAS:
 Purpose:https:
...
003    02/02/2024       glp110                  344896              add new Result  POC Rapid COVID-19 PCR
001    03/25/2024       Michael Mayes           PEND                Preventing filesize seqfault.
004    11/11/2024       Michael Mayes           239760              Only positives going out now.
*************END OF ALL MODCONTROL BLOCKS* *******************************************************************/
drop program doh_covid19_results_dc go
create program doh_covid19_results_dc
...
SET FILE_NAME =  concat("/cerner/d_p41/cust_output_2/doh_covid_results/medstar_dc-elramb-",format(cnvtdatetime(curdate,curtime3),"YYYYMMDDhhmmss;;Q"), ".csv")
    ;build2("medstar_DC", dataDate,".csv")
...
IF($TYPE = 3);EMAILING OF REPORT
    ...
    SET EMAIL_SUBJECT = build2("DC DOH ELR Report")
    ...
    SET EMAIL_BODY = concat("jwd_covid_dcelr_", format(cnvtdatetime(curdate, curtime3),"YYYYMMDDhhmmss;;Q"), ".dat")

    DECLARE FILENAME = VC
            WITH  NOCONSTANT(CONCAT("jwd_covid_dcelr_",
    ...
    IF ($TYPE = 3 and CURDOMAIN = PRODUCTION_DOMAIN)
        Select into (value(EMAIL_BODY))
            build2("The DC DOH ELR Report is attached to this email.", char(13), char(10), char(13), char(10),
...
join ph where ph.parent_entity_id = outerjoin(p.person_id)
 ...
 join a where a.parent_entity_id = outerjoin(e.organization_id)
...
STAT=ALTERLIST(RS->QUAL,PATIENTS)
    ...
    rs->QUAL[patients]->Perform_location = org.org_name
    rs->QUAL[patients]->perform_addr = a.street_addr
    rs->QUAL[patients]->perform_addr2 = a.street_addr2
    rs->QUAL[patients]->perform_city = a.city
    rs->QUAL[patients]->perform_state = a.state
    rs->QUAL[patients]->perform_zip = a.zipcode
    rs->QUAL[patients]->perform_county = uar_get_code_display(A.county_cd)
...
with nocounter, time = 1000, ORAHINTCBO("INDEX( E XIE18CLINICAL_EVENT)")
/****************************************************************************************************
                results
*****************************************************************************************************/
Select into "nl:"
...
join cep where cep.event_id = outerjoin(c.event_id)
...
head d.seq
    ...
    rs->QUAL[d.seq].result_action = uar_get_code_display(cep.action_type_cd)
        ...
        if (rs->QUAL[d.seq]->Perform_location not in( "*LabCorp*","*Quest*"))
...
join cep where cep.event_id = outerjoin(c.event_id)
;and (cep.action_type_cd = 103.00 ; order
;or cep.action_type_cd = 98.00) ; correct
...
head d.seq
 if(c.event_end_dt_tm > cnvtdatetime((curdate-1), 000000)
        AND c.event_end_dt_tm < cnvtdatetime((curdate-1), 235959));cep.action_type_cd = 678654.00 and
        rs->QUAL[d.seq]->same_day = "Y"
 endif
    rs->QUAL[d.seq]->PerformDtTm = format(c.performed_dt_tm, "MM/DD/YYYY hh:mm:ss;;D")
    rs->QUAL[d.seq]->SpecimenP = uar_get_code_display(c.event_cd)
    rs->QUAL[d.seq]->event = uar_get_code_display(c.event_cd)
...
with nocounter, format, time = 1000
;/********************************************************************************************
...
;                AND P.PROBLEM_ID != 0.00
;                AND P.CONFIRMATION_STATUS_CD = 3305.00 ;CONFIRMED
;                AND P.CLASSIFICATION_CD = 194037923.00 ;INFECTION PREVENTION
;                AND P.LIFE_CYCLE_STATUS_CD = 3301.00 ;ACTIVE
;                AND P.END_EFFECTIVE_DT_TM > cnvtdatetime(curdate,curtime3)
;                AND P.PROBLEM_INSTANCE_ID = (select max(PX.PROBLEM_INSTANCE_ID)
;                                from PROBLEM PX
;                                where PX.PERSON_ID = P.PERSON_ID
;                                   and PX.PROBLEM_ID = P.PROBLEM_ID
;                                   and PX.CONFIRMATION_STATUS_CD = P.CONFIRMATION_STATUS_CD
;                                   and PX.CLASSIFICATION_CD = P.CLASSIFICATION_CD
;                                   and PX.LIFE_CYCLE_STATUS_CD = P.LIFE_CYCLE_STATUS_CD
;                                   and PX.END_EFFECTIVE_DT_TM > cnvtdatetime(curdate,curtime3))
;JOIN N WHERE N.NOMENCLATURE_ID = P.NOMENCLATURE_ID
;                AND N.SOURCE_IDENTIFIER = "3902339012" ;SNOMED-CT: 2019 novel coronavirus
;
;detail
;
;rs->QUAL[D.SEQ].onset_dt = format(p.onset_dt_tm, "MM/DD/YYYY;;q")
;
...
with nocounter,time = 1000, ORAHINTCBO("INDEX( POR XIE1PERSON_ORG_RELTN)")
...
; if (size(rs->QUAL,5) > 0); AT LEAST ONE PATIENT FOUND ABOVE
 ...
 SELECT DISTINCT INTO VALUE(FILE_NAME)
   RESULT = if(rs->QUAL[D1.SEQ].Result != " ")
    ...
    , PERFORM_CLIA = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].PerformingCLIA))
...
;   , ADDITIONAL_NOTES= "NA";"TBD"
    ...
    where SIZE(RS->QUAL,5) > 0
      ...
      and rs->QUAL[d1.seq].ENCNTR_CD IN (
...
;   and rs->QUAL[d1.seq].Result != "Invalid"
   ;and rs->QUAL[d1.seq].Result in ("POS*","NEG*","NOT DET*","DET*","NOT-DET*")
    and rs->QUAL[d1.seq].Result in ("POS*",                  "DET*"           )  ;004
    and rs->QUAL[d1.seq].Perform_Location not in ( "*LabCorp*", "*Labcorp*")
    and rs->QUAL[d1.seq].Perform_Location != "*Quest Diagnostics*"
...
;   , OBESITY= "NA";trim(substring(1,30,RS->QUAL[d1.seq].OBESITY))
;   , ADDITIONAL_NOTES= "NA";"TBD"
    , order_id = rs->QUAL[D1.SEQ].OrderId
    , action_type_cd = rs->QUAL[D1.SEQ].result_action
    , perform_dttm = rs->QUAL[D1.SEQ].PerformDtTm
    , same_day = rs->QUAL[D1.SEQ].same_day
...
;   and rs->QUAL[d1.seq].Result != "Invalid"
   ;and rs->QUAL[d1.seq].Result in ("POS*","NEG*","NOT DET*","DET*","NOT-DET*")
    ...
    and rs->QUAL[d1.seq].Perform_Location not in ( "*LabCorp*", "*Labcorp*");
    and rs->QUAL[d1.seq].Perform_Location != "*Quest Diagnostics*"
    ;and rs->QUAL[d1.seq].Result != " "
    ;and rs->QUAL[D1.SEQ].result_action != "Endorse"
...
;;          CALL DCL(AIX_COMMAND,AIX_CMDLEN, AIX_CMDSTATUS)
        ...
        set EMAIL_SUBJECT = "COVID-19 DC DOH ELR -No accounts found"
