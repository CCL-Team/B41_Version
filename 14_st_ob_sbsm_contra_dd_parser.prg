/*************************************************************************
 Program Title:   SBSM Last Doc Contraception Plan after Delivery
 
 Object name:     14_st_ob_sbsm_contra_dd_parser
 Source file:     14_st_ob_sbsm_contra_dd_parser.prg
 
 Purpose:         Find previous SBSM Office note, and pull out the values
                  For the Contraception Plan after Delivery section.
                  
                  We display the selected results as a ST.
...
*************END OF ALL MODCONTROL BLOCKS* *******************************/
  drop program 14_st_ob_sbsm_contra_dd_parser:dba go
create program 14_st_ob_sbsm_contra_dd_parser:dba
...
declare contra_section    = vc  with protect, noconstant('')
...
if(blob_events->cnt > 0)
    ...
    set contra_section = parse_dyn_doc_section('>Contraception Plan After Delivery<', blob)


    call parse_drop_list(drop_list, contra_section)
...
if(blob_events->cnt > 0)
    set tmp_str = notrim(build2(wu, 'Last Documented Contraception Plan After Delivery', wr, reol))
