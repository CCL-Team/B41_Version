...
DROP PROGRAM hi_http_proxy_post_request:dba GO
CREATE PROGRAM hi_http_proxy_post_request:dba
 PROMPT
  "Output to File/Printer/MINE " = "MINE",
  "HTTP_ENDPOINT: " = "",
  "DATATYPE (XML, JSON, or RECORD): " = "RECORD",
  ...
  pat_person_id, usr_person_id, body,
  content_type, template_variables
 ...
 SUBROUTINE callproxy(proxyvalues)
   ...
   ENDIF
 END ;Subroutine
 FREE RECORD postproxyvalues
 RECORD postproxyvalues(
   1 endpoint = vc
   1 method = vc
   1 datatype = vc
   1 patientid = f8
 ...
 SET postproxyvalues->endpoint =  $HTTP_ENDPOINT
 SET postproxyvalues->method = "POST"
 SET postproxyvalues->datatype =  $DATATYPE
 SET postproxyvalues->patientid =  $PAT_PERSON_ID
 SET postproxyvalues->userid =  $USR_PERSON_ID
 SET postproxyvalues->templatevariables =  $TEMPLATE_VARIABLES
 SET postproxyvalues->body =  $BODY
 SET postproxyvalues->contenttype =  $CONTENT_TYPE
 CALL callproxy(postproxyvalues)
 IF (( $DATATYPE="JSON"))
