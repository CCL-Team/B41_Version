/************************************************************************************************************
 Program Title:     doh_covid19_results_elr.prg
 Create Date:       03/11/2020
 Object name:       doh_covid19_results_elr
 Source file:       doh_covid19_results_elr.prg
 MCGA:
 OPAS:
 Purpose:https:
...
Disclaimers:
Multiple rows/orders per patient as multiple orders for labs are placed on each encounter - rare but will likely
                                                                                                continue to grow
...
wrapper file : doh_covid19_results_wrapper - file
...
001    07/27/2020       Jwd107                                      HHS changes
002    07/20/2021       jwd107                  228841              add result snowmed codes
003    10/21/2021       jwd107                  230184              remove Invalid POC results
004    02/02/2024       glp110                  344896              add new Result  POC Rapid COVID-19 PCR
005    11/11/2024       Michael Mayes           239760              Only positives going out now.
*************END OF ALL MODCONTROL BLOCKS* *******************************************************************/
drop program doh_covid19_results_elr go
create program doh_covid19_results_elr
...
SET FILE_NAME =  concat("/cerner/d_p41/cust_output_2/doh_covid_results/md_elr_",format(cnvtdatetime(curdate,curtime3),"YYYYMMDDhhmmss;;Q"), ".csv")
    ;build2("medstar_MDTEST", dataDate,".csv")
...
IF($TYPE = 3);EMAILING OF REPORT
    ...
    SET EMAIL_SUBJECT = build2("[ENCRYPT]MedStar MD DOH ELR Results Report")
    ...
    SET EMAIL_BODY = concat("medstar_md_elr_",
    format(cnvtdatetime(curdate, curtime3),"YYYYMMDDhhmmss;;Q"), ".dat")

    DECLARE FILENAME = VC
            WITH  NOCONSTANT(CONCAT("medstar_md_elr_",
    ...
    build2("The MedStar MD DOH ELR Covid19 Results report is attached to this email.",
...
;*****************************************************************************************************
    ...
    join e where e.encntr_id = outerjoin(c3.encntr_id); and e.contributor_system_cd not in  (1038645461.00,  926194249.00); EMRLINK_ADD/demo
...
head d.seq
    ...
    rs->QUAL[d.seq]->service_resource = uar_get_code_display(c.resource_cd)
        ...
        rs->QUAL[d.seq]->PerformingCLIA = orga.alias
            ...
            if( c.event_cd = 5015944927.00);POC Rapid COVID-19 PCR
            rs->qual[d.seq].ACCESSION = cnvtstring(c.order_id)
            endif
...
; if (size(rs->QUAL,5) > 0); AT LEAST ONE PATIENT FOUND ABOVE
 ...
 SELECT DISTINCT INTO VALUE(FILE_NAME)
    ...
    where rs->QUAL[d1.seq].ENCNTR_CD IN (5045671.00 ;Recurring Outpatient
    ...
    ;and rs->QUAL[D1.SEQ].contributor not in (1038645461.00,  926194249.00);    EMRLINK_ADD/demo
    and (rs->QUAL[d1.seq].State_cd = "MD" or rs->QUAL[d1.seq].fac_State = "MD")
;   AND rs->QUAL[D1.SEQ].Result != "See Comment"
;    AND rs->QUAL[D1.SEQ].Result != "SEE COMMENT"
;    AND rs->QUAL[D1.SEQ].Result != "see comment"
...
;   and rs->QUAL[d1.seq].Result != "INVALID"
   ;and rs->QUAL[d1.seq].Result in ("POS*","NEG*","NOT DET*","DET*","NOT-DET*")
    and rs->QUAL[d1.seq].Result in ("POS*",                   "DET*"           )  ;005
    and rs->QUAL[d1.seq].Perform_Location not in ( "*LabCorp*","*Labcorp*")
    and rs->QUAL[d1.seq].Perform_Location != "*Quest Diagnostics*"
    and rs->QUAL[d1.seq].service_resource != "GSH ARUP SO"
    ;and rs->QUAL[d1.seq].Clinic_Name != "St. Mary's"
