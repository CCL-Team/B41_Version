/***********************************************************************************************************
 Program Title:     3_mp_results_md_ip.prg
 Create Date:       08/11/2022
 Object name:       3_mp_results_md_ip
 Source file:       3_mp_results_md_ip.prg
 MCGA:
 OPAS:
 Purpose:https:
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^IMPORTANT^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
*************************************************************************************************************
wrapper: doh_covid19_case_wrapper - email
...
drop program 3_mp_results_md_ip go
create program 3_mp_results_md_ip
...
SET FILE_NAME =  concat("/cerner/d_p41/cust_output_2/doh_covid_results/medstar_md_monkeypox_"
,format(cnvtdatetime(curdate,curtime3),"YYYYMMDDhhmmss;;Q"), ".csv")
;   build2("medstar_DC_mp_results", dataDate,".csv");medstar_dc-caseip-"
...
IF($TYPE = 3);EMAILING OF REPORT
    ...
    SET EMAIL_SUBJECT = build2("MedStar MD DOH Monkeypox Positive Result Report")
    ...
    SET EMAIL_BODY = concat("msh_pos_mpx_result_md_", format(cnvtdatetime(curdate, curtime3),"YYYYMMDDhhmmss;;Q"), ".dat")
 
    DECLARE FILENAME = VC
            WITH  NOCONSTANT(CONCAT("msh_pos_mpx_result_md_",
    ...
    IF ($TYPE = 3 and CURDOMAIN = PRODUCTION_DOMAIN)
        Select into (value(EMAIL_BODY))
            build2("The MedStar MD DOH Monkeypox Positive Results Report ",
...
; if (size(rs->QUAL,5) > 0); AT LEAST ONE PATIENT FOUND ABOVE
 ...
 SELECT INTO VALUE(FILE_NAME)
    ...
    where rs->QUAL[d1.seq].fac_State = "MD"
...
;;          CALL DCL(AIX_COMMAND,AIX_CMDLEN, AIX_CMDSTATUS)
        ...
        set EMAIL_SUBJECT = "The MedStar MD DOH Monkeypox Positive Result Report -No accounts found"
