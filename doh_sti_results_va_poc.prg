/***********************************************************************************************************
 Program Title:		doh_sti_results_va_poc.prg
 Create Date:		08/31/2022
 Object name:		doh_sti_results_va_poc
 Source file:		doh_sti_results_va_poc.prg
 MCGA:
 OPAS:
 Purpose:https:
 Executed from:		Explorer Menu
 Special Notes:		wrapper - doh_covid19_case_wrapper.prg - email
 					wrapper - doh_covid19_results_wrapper - file for pick up and delivery to VDH
...
---    ----------       --------------------    ------        		------------------------------------------
N/A    08/31/2022		Jeremy Daniel         	N/A	          		Initial Release
001    10/24/2024       Michael Mayes           239760              Adding new clinic
 
*************END OF ALL MODCONTROL BLOCKS* *******************************************************************/
drop program doh_sti_results_va_poc go
create program doh_sti_results_va_poc
...
SET FILE_NAME =  concat("/cerner/d_p41/cust_output_2/doh_covid_results/medstar_va_sti_",format(cnvtdatetime(curdate,curtime3),"YYYYMMDDhhmmss;;Q"), ".txt")
...
IF($TYPE = 3);EMAILING OF REPORT
	...
	SET EMAIL_SUBJECT = build2("[ENCRYPT]MedStar VA STI POC Report")
	...
	SET EMAIL_BODY = concat("medstar_vasti_poc_",
	format(cnvtdatetime(curdate, curtime3),"YYYYMMDDhhmmss;;Q"), ".dat")
 
	DECLARE FILENAME = VC
			WITH  NOCONSTANT(CONCAT("medstar_vasti_poc_",
...
plan c3 where c3.event_cd in (
				 3715370811.00	;POC Chlamydia PCR by Visby Med
,				 3715377891.00	;POC Gonorrhoeae PCR by Visby Med
;,				 3715379935.00	;POC Trichomonas PCR by Visby Med ;***new***
	)
...
and org.organization_id in (6114269.00,6276591,18933272.00)   ;001
...
STAT=ALTERLIST(RS->QUAL,PATIENTS)
	...
	rs->QUAL[patients]->event_cd = c3.event_cd ;***new***

	 if(rs->QUAL[patients]->event_cd in (2276648185 ; NEW POC ORDER
										,2404008691)) ;POC Ag											
	 rs->QUAL[patients]->PersonId = c3.person_id
	 rs->QUAL[patients]->EncntrId = c3.encntr_id
	 rs->QUAL[patients]->hospitalized = "No"
	 rs->QUAL[patients]->ICU = "No"
	 else
	 rs->QUAL[patients]->sti_PersonId = c3.person_id
	 rs->QUAL[patients]->sti_EncntrId = c3.encntr_id
	 endif
 												;***new***
    ...
    rs->QUAL[patients]->CMRN = PA.alias
 	...
 	;RS->QUAL[patients]->oc_stat = "Unknown"
		...
		;***new***
		if(c3.event_cd in ( 3715370811.00	;POC Chlamydia PCR by Visby Med
							,3715377891.00	;POC Gonorrhoeae PCR by Visby Med
							;,3715379935.00 ;POC Trichomonas PCR by Visby Med)
							))	
		rs->QUAL[patients]->Specimen = "Vaginal swab (specimen)"
		rs->QUAL[patients]->Specimen_type = "258520000"
		rs->QUAL[patients]->Specimen_ns = "SCT"
		;rs->QUAL[patients]->service_resource = "Visby"
		rs->qual[patients].ACCESSION = cnvtstring(c3.order_id)
		;rs->QUAL[patients]->EAU_TEST_KIT = "Visby"
		rs->qual[patients].result_status = "F"
 		rs->qual[patients].TEST_RESULT_STATUS = "F"
 		rs->qual[patients].order_dttm = c3.verified_dt_tm
		rs->QUAL[patients]->o_loinc = "92683-2"
		rs->QUAL[patients]->o_event = "Chlamydia trachomatis and Neisseria gonorrhoeae and Trichomonas vaginalis DNA panel - Genital specimen by NAA with probe detection"
		;rs->QUAL[patients]->loinc = "94558-4"
		endif
		
		if(c3.event_cd  = 3715370811.00)	;POC Chlamydia PCR by Visby Med
		rs->qual[patients].ACCESSION = concat(cnvtstring(c3.order_id),"_6356-0")
		endif
		
		if(c3.event_cd  = 3715377891.00)	;POC Gonorrhoeae PCR by Visby Med
		rs->qual[patients].ACCESSION = concat(cnvtstring(c3.order_id),"_47387-6")
		endif
 		;***new***
 
with nocounter, time = 1700, ORAHINTCBO("INDEX( E XIE18CLINICAL_EVENT)")
...
head d.seq
    ...
    rs->QUAL[d.seq].RESULT_DT = format(C.valid_from_dt_tm, "YYYYMMDDhhssmm;;D")
 	...
 	elseif(c.event_cd = 3715370811.00)
 	rs->QUAL[d.seq]->event = "Chlamydia trachomatis DNA [Presence] in Genital specimen by NAA with probe detection"
 	elseif(c.event_cd = 3715377891.00)
 	rs->QUAL[d.seq]->event = "Neisseria gonorrhoeae DNA [Presence] in Genital specimen by NAA with probe detection"
 	endif
...
PLAN d
	...
	join e WHERE e.encntr_id in( rs->qual[d.seq].EncntrId, rs->qual[d.seq].sti_EncntrId)
	join org where org.organization_id = outerjoin(e.organization_id)
...
if(rs->QUAL[d.seq].Specimen = "Vag*")
rs->QUAL[d.seq].Specimen = "Vaginal swab (specimen)"
endif
...
PLAN d
	...
	join A WHERE A.parent_entity_id in( rs->qual[d.seq].PERSONID,rs->qual[d.seq].sti_personId)
...
; 	elseif(datetimediff(e.reg_dt_tm, p.birth_dt_tm) < 28 and datetimediff(e.reg_dt_tm, p.birth_dt_tm) > 1 )
; 	rs->QUAL[d.seq]->AGE = datetimediff(e.reg_dt_tm, p.birth_dt_tm)/365
; 	rs->QUAL[d.seq]->age_unit = "Days"
; 	elseif(datetimediff(e.reg_dt_tm, p.birth_dt_tm) < 1 and datetimediff(e.reg_dt_tm, p.birth_dt_tm) > 0 )
; 	rs->QUAL[d.seq]->AGE = datetimediff(e.reg_dt_tm, p.birth_dt_tm)/365/24
; 	rs->QUAL[d.seq]->age_unit = "Hours"
; 	else
; 	rs->QUAL[d.seq]->AGE = datetimediff(e.reg_dt_tm, p.birth_dt_tm)/365/24
; 	rs->QUAL[d.seq]->age_unit = " "
; 	endif
; 
;with nocounter, time = 1200
/********************************************************************************************
...
if($Type = 2)
 ...
 SELECT DISTINCT INTO VALUE(FILE_NAME)
	...
	, ORDER_CODE_TEXT_DESCRIPTION = trim(SUBSTRING(1, 130, rs->QUAL[D1.SEQ].EVENT));***new***
	, ORDER_CODE_NAMING_SYSTEM = "LN"
	, RESULT_VALUE_TYPE = "CE"
	, RESULT_TEST_CODE = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].LOINC))
	, RESULT_TEST_TEXT_DESCRIPTION = trim(SUBSTRING(1, 130, rs->QUAL[D1.SEQ].EVENT));***new***
	...
	, HOSPITALIZED = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].hospitalized))
	, ICU = trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].icu))
...
;	, LOINC = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].LOINC))
 ...
 if (size(rs->QUAL,5) > 0)
 	...
 	;if(rs->QUAL[D1.SEQ].service_resource in ("POC Abbot ID NOW_Abbott_DIT","BD Veritor Plus"))
	...
	, RESULT_VALUE_TYPE = "CE"
	, RESULT_TEST_CODE = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].LOINC))
	, RESULT_TEST_TEXT_DESCRIPTION = trim(SUBSTRING(1, 130, rs->QUAL[D1.SEQ].EVENT));******
	, RESULT_TEST_NAMING_SYSTEM = "LN"
	, OBSERVATION_VALUE = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].result_sc))
	, OBSERVATION_VALUE_RESULT_TEXT = trim(SUBSTRING(1, 100, rs->QUAL[D1.SEQ].Result))
	, OBSERVATION_VALUE_RESULT_NAMING_SYSTEM = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].result_ns))
...
;	if(rs->QUAL[D1.SEQ].service_resource in ("POC Abbot ID NOW_Abbott_DIT","BD Veritor Plus"))
	...
	, EAU_TEST_KIT_IDENTIFIER = trim(SUBSTRING(1, 100, rs->QUAL[D1.SEQ].EAU_TEST_KIT));******
	, MODEL_NAME_TESTKIT_IDENTIFIER = " "
	, DEVICE_IDENTIFIER = " ";trim(SUBSTRING(1, 20, rs->QUAL[D1.SEQ].service_resource))
	, MODEL_NAME_BASED_INSTRUMENT = " "
	, DEVICE_IDENTIFIER_BASED_INSTRUMENT = " "
	...
	and rs->QUAL[d1.seq].Result != "QNS"
	and rs->QUAL[d1.seq].Result != "Invalid"
	;and rs->QUAL[D1.SEQ].service_resource in ("POC Abbot ID NOW_Abbott_DIT","BD Veritor Plus")
