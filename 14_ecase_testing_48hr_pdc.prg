/************************************************************************************************************
 Program Title:     14_ecase_testing_48hr_pdc.prg
 Create Date:       07/11/2022
 Object name:       14_ecase_testing_48hr_pdc
 Source file:       14_ecase_testing_48hr_pdc.prg
 MCGA:
 OPAS:
 Purpose:https:
...
(   1 PRINTCNT = i4
    1 Qual[*]
        ...
        2 action_event = vc
        2 valid_from_dttm = dq8
        2 clin_sig_dttm = dq8
        2 early_result_dt = dq8
        2 last_result_dt = dq8
        2 INCLUDE = I2
        2 RESULT_DTTM_DIFF = I4
        2 early_result = vc
        2 last_result = vc
        2 UPDATE_COUNT = I4
)
...
IF($TYPE = 3);EMAILING OF REPORT
    ;002-> Major refactoring here to allow build saves to TEAMS
...
STAT=ALTERLIST(RS->QUAL,PATIENTS)
    ...
    rs->QUAL[patients]->valid_from_dttm = cnvtdate(c3.valid_from_dt_tm,"MM/DD/YYYY;;D")
...
JOIN c where c.event_id = rs->qual[d.seq].event_id
        ...
        ;and c3.result_val != "Invalid"
        and c.event_end_dt_tm > cnvtlookbehind("14,D")
...
; if (size(rs->QUAL,5) > 0); AT LEAST ONE PATIENT FOUND ABOVE
 ...
 if (size(rs->QUAL,5) > 0)
    ...
    , REG_DATE = format(RS->QUAL[d1.seq].reg_dt, "MM/DD/YYYY hh:mm:ss;;D")
    , DISCHARGE_DATE = format(RS->QUAL[d1.seq].discharge_dttm, "MM/DD/YYYY hh:mm:ss;;D")
    , action_event = rs->QUAL[d1.seq].action_event
    ;, CLIN_SIG_DATE = format(RS->QUAL[d1.seq].clin_sig_dttm, "MM/DD/YYYY hh:mm:ss;;D")
    , EARLY_RESULT_DATE = format(RS->QUAL[d1.seq].EARLY_RESULT_DT, "MM/DD/YYYY hh:mm:ss;;D")
    , EARLY_RESULT = trim(SUBSTRING(1, 30, rs->QUAL[D1.SEQ].early_result))
    , LAST_RESULT_DATE = format(RS->QUAL[d1.seq].last_result_dt, "MM/DD/YYYY hh:mm:ss;;D")
