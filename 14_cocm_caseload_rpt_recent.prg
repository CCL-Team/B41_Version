...
drop   program 14_cocm_caseload_rpt_recent:dba go
create program 14_cocm_caseload_rpt_recent:dba
...
select into "nl:"
  ...
  plan d 
   where d.dcp_forms_ref_id        in (27852023149.00, 25562326477.00, 25562361699.00, 13047318247.00)  ; 002 Adding CoCM Form
     and d.active_ind              =  1
  ...
  join ce
   where ce.parent_event_id        =  dfc.parent_entity_id
     ...
     and (   (ce.event_cd = 4824728385.00  and d.dcp_forms_ref_id = 25562361699.00)
          ...
          or (ce.event_cd = 2417652201     and d.dcp_forms_ref_id = 13047318247.00)  ; 002 Adding CoCM Form
         )
  ...
  ;002->  Moving up here and killing outerjoin
  join epb                        
   where epb.PARENT_EVENT_id      =  ce5.parent_event_id
     and epb.valid_until_dt_tm    >= cnvtdatetime(curdate,curtime3)
     and epb.view_level           =  1
     and epb.event_cd             =  2417619091.00;Date Episode of Care begin
                                  
  join cdrb                       
   where cdrb.event_id            =  epb.event_id
     and cdrb.valid_until_dt_tm   >= cnvtdatetime(curdate,curtime3)
  ;002<-
  ...
  ;002->
  ;For this report, we only want Episode of case after Jan-04-2024
  ;join epb                        
...
head eoc
    ...
    if(cdrb.result_dt_tm        >= cnvtdatetime('04-JAN-2024 00:00:00'))
        cnt = cnt + 1
...
if(size(reply->items,5)>0)
    ...
    head ce5.event_cd
        ...
        ;We have to attach this to all episodes of care served by this form... 
        for(looper = 1 to size(reply->items,5))
            if(reply->items[looper].ce_parent_event_id = ce.parent_event_id)
                
                case(ce5.event_cd)
                ;CoCM flag
                of 2417629549.00: 
                    reply->items[looper].flag                = ce5.result_val
                ;Date of Last Psych Consult
                of 2417628261.00: 
                  reply->items[looper].last_physc_cnst       = format(cdr.result_dt_tm, "MM/DD/YYYY")
                  reply->items[looper].since_last_physc_cnst = floor( datetimediff( cnvtdatetime(curdate, curtime3)
                ...
                of 2417619797.00: 
                    reply->items[looper].treatmnt_status       = ce5.result_val
                    reply->items[looper].treatmnt_status_date  = format(ce.performed_dt_tm, "mm/dd/yyyy")
                                  
                of 3395348155.00: 
                    reply->items[looper].visit_freq       = cnvtalphanum(ce5.result_val, 1)
                    reply->items[looper].visit_freq_lbl   = ce5.result_val
                    date_lbl                              = build2(trim(reply->items[looper].visit_freq), " W")
                    reply->items[looper].visit_freq       = date_lbl
                    reply->items[looper].next_fu_due_date = format(cnvtlookahead( date_lbl
                                                                                , cnvtdatetime(reply->items[looper].p_reg_date_dq8))
                                                                                , "mm/dd/yyyy;;d"
                                                                  )
                of 4986730465.00: 
                    reply->items[looper].care_manager          = ce5.result_val
                of 4202997599.00: 
                    reply->items[looper].target_scale          = ce5.result_val
                endcase
            endif
        endfor
    ...
    select into 'nl:'
        eoc = reply->items[d.seq].episode_of_care
    ...
    order by ce5.person_id, eoc, ce5.event_cd, ce5.event_end_dt_tm desc
    ...
    join ce3
     where ce3.parent_event_id       =  ce2.event_id
       ...
       and ce3.event_end_dt_tm    >= cnvtdatetime(cnvtdate(reply->items[d.seq].episode_begin), 0)
       and (   (    reply->items[d.seq].episode_end >  0 
                and ce3.event_end_dt_tm          <= cnvtdatetime(cnvtdate(reply->items[d.seq].episode_end), 235959)
               ) 
            or reply->items[d.seq].episode_end = 0 
            ;002<-
           )
    ...
    head ce3.event_cd
        ...
        of 102264072.00: reply->items[d.seq].initial_phq9_score   = ce3.result_val
                         reply->items[d.seq].initial_phq9_date    = format(ce3.event_end_dt_tm, "mm/dd/yyyy")
        of 823726295.00: reply->items[d.seq].initial_gad7_score   = ce3.result_val
                         reply->items[d.seq].initial_gad7_date    = format(ce3.event_end_dt_tm, "mm/dd/yyyy")
        of 712404847.00: reply->items[d.seq].initial_auditc_score = ce3.result_val
                         reply->items[d.seq].initial_auditc_date  = format(ce3.event_end_dt_tm, "mm/dd/yyyy")
        endcase
    
    foot ce3.event_cd
        case(ce3.event_cd)
        of 102264072.00: reply->items[d.seq].last_phq9_score      = ce3.result_val
                         reply->items[d.seq].last_phq9_date       = format(ce3.event_end_dt_tm, "mm/dd/yyyy")
        
        of 823726295.00: reply->items[d.seq].last_gad7_score      = ce3.result_val
                         reply->items[d.seq].last_gad7_date       = format(ce3.event_end_dt_tm, "mm/dd/yyyy")
        
        of 712404847.00: reply->items[d.seq].last_auditc_score    = ce3.result_val
                         reply->items[d.seq].last_auditc_date     = format(ce3.event_end_dt_tm, "mm/dd/yyyy")
        endcase
