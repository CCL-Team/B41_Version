/*************************************************************************
 Program Title:   CC Medication Continuous Infusions Autotext (All)
 
 Object name:     5_st_cc_cont_inf_all_auto
 Source file:     5_st_cc_cont_inf_all_auto.prg
 
 Purpose:         This ST is supposed to be a subset of the 5_st_cc_inpat_meds, pulling only
                  the currently active IV meds from it.
                  
                  This will be used in autotext (.medicationContinuousInfusionAllCritCare)
...
*************END OF ALL MODCONTROL BLOCKS* *******************************/
  drop program 5_st_cc_cont_inf_all_auto:dba go
create program 5_st_cc_cont_inf_all_auto:dba
...
execute 5_st_cc_cont_inf_all:dba with replace(request, st_request), replace(reply, st_reply)
...
if(st_reply->text = empty_str)
    ...
    set reply->text = replace(st_reply->text, rhead
                                         , notrim(build2( rhead
                                                        , wbu, 'Continuous Infusions (Active)', wr
                                                        , reol
                                                        )
                                                 )
                             )
